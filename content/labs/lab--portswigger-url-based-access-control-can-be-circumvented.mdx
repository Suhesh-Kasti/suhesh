...
tags:
  ...
  - access-control
  - walkthrough
excerpt: "A front-end rule blocks external access to /admin, but the backend trusts the X-Original-URL header and I walk right past it."
type: lab
...
- [ ] Step 2:
- [ ] Step 3:
![Request to the root path carrying an X-Original-URL header that points at /admin](/images/content/lab--portswigger-url-based-access-control-can-be-circumvented/req_with_x-original-url.webp)
- [ ] Final payload / exploit:
![The delete-user request succeeding through the X-Original-URL header bypass](/images/content/lab--portswigger-url-based-access-control-can-be-circumvented/username_deleted_with_x-original-url.webp)
...
<Cloze title="Lab recall" color="#00e5ff" text="The front end blocks /admin, but the back-end framework trusts the {{X-Original-URL}} header, so pointing it at /admin reaches the panel and lets you delete {{carlos}}." />

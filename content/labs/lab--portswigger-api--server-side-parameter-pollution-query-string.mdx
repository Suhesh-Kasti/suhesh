---
title: Server side parameter pollution in query string
date: '2026-07-08'
tags:
  - PortSwigger
  - lab
  - api-testing
  - walkthrough
excerpt: "The forgot-password form injects my username into a backend query string, so an encoded hash truncates it and unhides a hidden field parameter."
type: lab
category: PortSwigger Academy
platform: PortSwigger
---

# Lab: Server side parameter pollution in query string
...
![Error response proving the injected parameter is being parsed by the backend](/images/content/shared/api-parameter-pollution-something-is-being-processed.webp)
...
![Error message stating that the field parameter is not set](/images/content/shared/api-parameter-pollution-field-not-specified.webp)
...
![Error message stating that the supplied value for the field parameter is invalid](/images/content/shared/api-parameter-pollution-invalid-field.webp)
...
![Intruder results showing the fuzzed server-side field name that returns a 200 response](/images/content/shared/Pasted%20image%2020260819154228.webp)
...
![The password-reset JavaScript revealing that a reset_token field is required](/images/content/shared/api-reset-token-required-js-code.webp)
...
![Response saying no reset token exists for the requested user](/images/content/shared/api-parameter-pollution-no-reset-token.webp)
![Response returning a valid reset token for the user carlos](/images/content/shared/api-parameter-pollution-carlos-reset_token.webp)
- [ ] Step 8:  Using that reset token I can change the password for the user![Changing the user's password by supplying the leaked reset token](/images/content/shared/api-parameter-pollution-change-password-using-reset_token.webp)
...
<Cloze title="Lab recall" color="#00e5ff" text="An encoded ampersand ({{%26}}) proves the backend parses the username as a parameter, and an encoded hash ({{%23}}) comments out the rest to expose a hidden {{field}} parameter." />

...
tags:
  ...
  - file-upload
  - walkthrough
excerpt: "The upload folder serves PHP as plain text, so a URL-encoded ../ in the filename drops my shell one directory up where it runs."
type: lab
...
<Cloze title="Lab recall" color="#00e5ff" text="The upload folder serves PHP as text, so a URL-encoded {{../}} in the filename drops the shell one directory up where it executes." />
